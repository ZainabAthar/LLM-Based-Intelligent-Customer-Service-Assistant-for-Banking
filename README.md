...
├── lora_bank_model_qwenx/       # Fine-tuned LoRA adapter weights
...
- **Python 3.10+**
- **ngrok account** (free) for tunneling: [ngrok.com](https://ngrok.com)
...
This creates the LoRA adapter in `lora_bank_model_qwenx/`. After training, back it up to Google Drive.

### Step 4: Start the API Backend (Colab)

```bash
python api_service.py --token {YOUR_NGROK_AUTH_TOKEN}
```
...
| `LORA_PATH` | `/content/drive/MyDrive/colab_deploy/lora_bank_model_qwenx` | LoRA adapter path |
