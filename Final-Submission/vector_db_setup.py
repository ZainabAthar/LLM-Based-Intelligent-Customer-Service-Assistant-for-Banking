...
def setup_vector_db():
    ...
    global INPUT_FILE
    # Smart path check for Colab
    if not os.path.exists(INPUT_FILE):
        alternate_paths = [
            os.path.join("/content", INPUT_FILE),
            os.path.join("/content/drive/MyDrive", INPUT_FILE),
            "enriched_bank_data.jsonl"
        ]
    ...
    if not os.path.exists(INPUT_FILE):
        ...
        print("  Tips for Colab:")
        print("  1. Run 'advanced_preprocessing.py' first to generate this file.")
        print("  2. Ensure Google Drive is mounted: drive.mount('/content/drive')")
        print("  3. Upload the file manually to the 'preprocessed_data' folder.")
        return
    ...
    print(f"  -> Total records stored: {final_count}")
    print(f"  -> Location: '{os.path.abspath(PERSIST_DIRECTORY)}'")
    if final_count > 0:
        print("  -> You can now run rag_assistant.py")
    else:
        print("  -> Warning: 0 records were indexed. Check your input file format.")
