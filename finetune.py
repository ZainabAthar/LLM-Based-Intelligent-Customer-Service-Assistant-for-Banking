...
OUTPUT_DIR     = "/content/lora_bank_model"   # Colab path
ENRICHED_JSONL = "preprocessed_data/enriched_bank_data.jsonl"
...
def load_enriched_jsonl(path):
    ...
    with open(path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            q = data["structured_data"]["question"].strip()
            a = data["structured_data"]["answer"].strip()
            if q and a:
...
training_args = TrainingArguments(
    ...
    report_to="none",
    dataloader_num_workers=2,       # Colab supports multiple workers unlike Windows
...
# ─────────────────────────────────────────────
#  CELL 4 — Download the trained adapter
