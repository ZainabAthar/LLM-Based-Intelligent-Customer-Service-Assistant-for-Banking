...
def extract():
    ...
    for i in range(len(data)):
        ...
        if left_val:
            # Header detection (Expanded for PWRA)
            if any(k in left_val for k in ["Account", "Savings", "LCA", "Pensioners", "PWRA", "PakWatan"]):
        ...
        if term_val:
            # Header detection (Expanded for PWRA)
            if any(k in term_val for k in ["SNDR", "Term Deposit", "Maximiser", "PWRA", "PakWatan"]):
